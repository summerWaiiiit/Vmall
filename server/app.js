...
app.get("/list", (req, res) => {
  ...
  var query = pool.query(
    "select * from red_book where type_id=? limit ?, ?",
    [id,offset, limit],
    (err, result) => {
      if (err) {
        console.log(err);
      ...
      pool.query(
        "select * from release_imgs where `content-id` in(?) and is_cover = 2",
        [Object.keys(list)],
        (err, result) => {
          console.log(result);
          for (let key in result) {
            if (result.hasOwnProperty(key)) {
              let info = result[key];
              if (
                info.hasOwnProperty("content-id") &&
                list.hasOwnProperty(info["content-id"])
              ) {
...
app.get("/content", (request, response) => {
  ...
  pool.query("select * from red_book where id = ?", [id], (err, result) => {
    ...
    pool.query(
      "select * from release_imgs where `content-id` = ?",
      [id],
      (err, result) => {
        if (err) {
          return response.send({ msg: "数据库异常" });
  ...
  request.get(
    ...
    function(error, response, body) {
      ...
      if (!apiResult.open_id || !apiResult.session_key || !apiResult.unionid) {
        pool.query(
          "select * from user_info where open_id = ? and unionid = ?",
          [apiResult.open_id, apiResult.unionid],
          (err, result) => {
            if (err) {
              console.log(err);
            ...
            if (result.length > 0) {
              ...
              pool.query(
                "update user_info set session_key = ?",
                [apiResult.session_key],
                (err, result) => {
                  if (err) {
                    console.log(err);
                    return res.send({ code: 1, msg: "数据库异常" });
              ...
              pool.query(
                ...
                { open_id: apiResult.open_id, unionid: apiResult.unionid },
                (err, result) => {
                  if (err) {
                    console.log(err);
                    return res.send({ code: 1, msg: "数据库异常" });
...
app.get("/favorite", (req, res) => {
  ...
  pool.query(
    "select * from user_info where session_key = ?",
...
app.post("/favorite", (req, res) => {
  ...
  pool.query(
    ...
    (err, results) => {
      ...
      let userInfo = results[0];
      pool.query(
        "select * from favorite where user_id = ? and content = ?",
        [userInfo.user_id, req.body.content_id],
        (err, results) => {
          if (err) {
            console.log(err);
            return res.send({ code: 1, msg: "数据库异常" });
          ...
          if (results.length > 0) {
            ...
            pool.query(
              "insert into favorite set user_id = ?, content_id = ?",
              [userInfo.user_id, req.body.content_id],
              (err, result) => {
                if (err) {
                  console.log(err);
